...
namespace PersonalBlog.Context
{
    public static class BlogPostRepository
    {
        static BlogPostRepository()
        {
            ...
            {
                AdicionarBlogPost(context, "How to capture posts via APIs", "How to use fetch to get a list of blog posts");
                AdicionarBlogPost(context, "Using Indexed DB", "How to save a list of posts using IndexedDB");
                //AdicionarBlogPost(context, "Clean Code: A Handbook of Agile Software Craftsmanship", "Robert C. Martin", 2008);
                //AdicionarBlogPost(context, "Implementing Domain-Driven Design", "Vaughn Vernon", 2013);
                //AdicionarBlogPost(context, "Patterns, Principles, and Practices of Domain-Driven Design", "Scott Millet", 2015);
                //AdicionarBlogPost(context, "Refactoring: Improving the Design of Existing Code", "Martin Fowler", 2012);
        ...
        private static void AdicionarBlogPost(ApiContext context, string texto, string shortdescription)
        {
            context.blogPostTestes.Add(new BlogPostTeste()
            {
                Title = texto,
        ...
        public static List<BlogPostTeste> ListarBlogPost()
        {
            using (var context = new ApiContext())
            {
                return context.blogPostTestes.OrderBy(c => c.Title).ToList();
            }
        }
    }
}
